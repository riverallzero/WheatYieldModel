...
def pick_dry_value():
    dry_file_list = ['../output/unbong/U_stem_DryWeight_0510.csv',
                 '../output/unbong/U_stem_DryWeight_0531.csv']
...
def pick_fresh_value():
    fresh_file_list = ['../output/unbong/U_stem_FreshWeight_0510.csv',
                 '../output/unbong/U_stem_FreshWeight_0531.csv']
...
def pick_water_value():
    water_file_list = ['../output/unbong/U_stem_WaterContent_0510.csv',
                 '../output/unbong/U_stem_WaterContent_0531.csv']
...
def save_stem():
    ...
    df['date_1'] = date_0510
    df['group'] = groups
    df['stem_dryweight_1'] = dry['dryweight_1']
    df['stem_freshweight_1'] = fresh['freshweight_1']
    df['stem_watercontent_1'] = water['watercontent_1']
    df['lai_1'] = lai[0]
    df['spad_1'] = spad[0]


    df['date_2'] = date_0531
    df['stem_dryweight_2'] = dry['dryweight_2']
    df['stem_freshweight_2'] = fresh['freshweight_2']
    df['stem_watercontent_2'] = water['watercontent_2']
    df['lai_2'] = lai[1]
    df['spad_2'] = spad[1]

    df['yield'] = yield_data['yield']

    df.to_csv(os.path.join(output_dir, 'stem_time_dataset.csv'), index=False, encoding='utf-8')
    ...
    dn['stem_freshweight'] = fresh_list
    ...
    dn['stem_watercontent'] = water_list
    ...
    dn.to_csv(os.path.join(output_dir, 'stem_dataset.csv'), index=False, encoding='utf-8')
